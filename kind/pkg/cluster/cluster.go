...
func (c *Context) Create() error {
	...
	clusterID, err := uid()
	if err != nil {
		return fmt.Errorf("error creating cluster uid: %v", err)
	}
	// TODO(bentheelder): multiple nodes
	return c.provisionNode(clusterID)
}
...
func (c *Context) provisionNode(clusterID string) error {
	// TODO(bentheelder): multiple nodes...
	nodeName := "kind-" + clusterID + "-master"
	...
	// however, we need other things from `docker run --priviliged` ...
...
func (c *Context) createNode(name string) error {
	...
	cmd.Args = append(cmd.Args,
		...
		"-t", // we need a psuedo-tty for systemd logs
		// running containers in a container requires priviliged
...
func (c *Context) deleteNode(name string) error {
...
func (c *Context) runOnNode(name string, command []string) error {
	// TODO(bentheelder): use config
	// TODO(bentheelder): logging
